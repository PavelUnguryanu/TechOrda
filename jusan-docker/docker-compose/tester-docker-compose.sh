...
is_container_present() {
    NLINES=$(docker ps -qa -f "name=jusan-compose" -f "status=running" -f "ancestor=jusan-fastapi-final:dockerized" | wc -l | sed "s/[[:space:]]//g")
    echo $NLINES
}

get_host_port() {
    HOST_PORT=$(docker container inspect jusan-compose | jq --raw-output '.[0]."HostConfig"."PortBindings"."8080/tcp"[0]."HostPort"')
    echo $HOST_PORT
}

is_port8080_present() {
    docker container inspect jusan-compose | grep "\"8080/tcp\": \[" > /dev/null
    echo $?
}
